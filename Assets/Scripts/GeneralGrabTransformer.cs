...
using static UnityEngine.XR.Interaction.Toolkit.Transformers.XRBaseGrabTransformer;
using ScaleMode = UnityEngine.XR.Interaction.Toolkit.ScaleMode;
#if BURST_PRESENT
using Unity.Burst;
#endif

public class GeneralGrabTransformer : XRBaseGrabTransformer
{
    /// <summary>
    ...
    [SerializeField]
    ManipulationAxes m_PermittedRotationAxis = ManipulationAxes.All;

    /// <summary>
    /// Permitted axes for rotation constraint.
    /// </summary>
    public ManipulationAxes permittedRotationAxis
    {
        get => m_PermittedRotationAxis;
        set => m_PermittedRotationAxis = value;
    }

    [Header("Custom Local Translation Constraints")]
    [SerializeField]
    ManipulationAxes m_CustomLocalTranslationAxes = ManipulationAxes.All;

    /// <summary>
    /// Axes allowed for movement in local space.
    /// Unlike m_PermittedDisplacementAxes, this can update per frame (based on transform).
    /// </summary>
    public ManipulationAxes customLocalTranslationAxes
    {
        get => m_CustomLocalTranslationAxes;
        set => m_CustomLocalTranslationAxes = value;
    }
    ...
    public override void Process(XRGrabInteractable grabInteractable, XRInteractionUpdateOrder.UpdatePhase updatePhase, ref Pose targetPose, ref Vector3 localScale)
    {
        ...
        if ((m_PermittedRotationAxis & GeneralGrabTransformer.ManipulationAxes.X) == 0)
            newRotationEuler.x = m_InitialEulerRotation.x;

        if ((m_PermittedRotationAxis & GeneralGrabTransformer.ManipulationAxes.Y) == 0)
            newRotationEuler.y = m_InitialEulerRotation.y;

        if ((m_PermittedRotationAxis & GeneralGrabTransformer.ManipulationAxes.Z) == 0)
            newRotationEuler.z = m_InitialEulerRotation.z;

        targetPose.rotation = Quaternion.Euler(newRotationEuler);

        //// ----- 自定义实时局部轴限制 -----
        //if (m_CustomLocalTranslationAxes != ManipulationAxes.All)
        //{
        //    Vector3 displacement = targetPose.position - m_OriginalObjectPose.position;

        //    Vector3 constrainedDisplacement = Vector3.zero;

        //    if ((m_CustomLocalTranslationAxes & ManipulationAxes.X) != 0)
        //        constrainedDisplacement += Vector3.Project(displacement, grabInteractable.transform.right);

        //    if ((m_CustomLocalTranslationAxes & ManipulationAxes.Y) != 0)
        //        constrainedDisplacement += Vector3.Project(displacement, grabInteractable.transform.up);

        //    if ((m_CustomLocalTranslationAxes & ManipulationAxes.Z) != 0)
        //        constrainedDisplacement += Vector3.Project(displacement, grabInteractable.transform.forward);

        //    targetPose.position = m_OriginalObjectPose.position + constrainedDisplacement;
        //}
        if (m_CustomLocalTranslationAxes != ManipulationAxes.All)
        {
            Vector3 worldDisplacement = targetPose.position - m_OriginalGrabPosition;

            // 转换为局部坐标空间中的位移量
            Vector3 localDisplacement = grabInteractable.transform.InverseTransformDirection(worldDisplacement);

            // 清除未允许的轴
            if ((m_CustomLocalTranslationAxes & ManipulationAxes.X) == 0)
                localDisplacement.x = 0f;
            if ((m_CustomLocalTranslationAxes & ManipulationAxes.Y) == 0)
                localDisplacement.y = 0f;
            if ((m_CustomLocalTranslationAxes & ManipulationAxes.Z) == 0)
                localDisplacement.z = 0f;

            // 转回世界坐标系
            Vector3 constrainedDisplacement = grabInteractable.transform.TransformDirection(localDisplacement);
            targetPose.position = m_OriginalGrabPosition + constrainedDisplacement;

            if (permittedRotationAxis == ManipulationAxes.None)
            {
                // 如果没有允许的旋转轴，则将目标旋转设置为初始旋转
                targetPose.rotation = m_OriginalGrabRotation;
            }
            else
            {
                // 如果有允许的旋转轴，则保持当前旋转
    ...
    public override void OnGrab(XRGrabInteractable grabInteractable)
    {
        ...
        //
        m_OriginalGrabPosition = grabInteractable.transform.position;
        m_OriginalGrabRotation = grabInteractable.transform.rotation;
